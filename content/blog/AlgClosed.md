...
In this blog post, I'll discuss a proof of the Fundamental Theorem of Algebra that I came up with last year.
...
$$i(\alpha) = \frac{1}{N(\alpha)}\prod_{\sigma \in \mathrm{Gal}(\bb{K}/\bb{R})} \sigma(\alpha)$$
...
**<span class = "both">Proof of Lemma 2:</span>** By the inverse function theorem, it suffices to show that for any $\alpha \in \bb{K}^{\times}$, the map $dF_{\alpha}: \bb{K} \to \bb{K}$ is an isomorphism of $\R$-vector spaces. Indeed, the derivative of the map $F: \alpha \mapsto \alpha^2$ is simply $dF_{\alpha}: x \mapsto 2\alpha x$, which is indeed an isomorphism of $\R$-vector spaces. $\square$
